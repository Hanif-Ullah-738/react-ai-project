  ...
  return (
    <div className={darkMode == 'dark' ? 'dark' : 'light'}>
      <div className="grid grid-cols-5 h-screen text-center">
        ...
        <select onChange={(event) => setDarkMode(event.target.value)} className="fixed bottom-5 left-5 dark:text-white text-black outline-0 dark:bg-zinc-800 bg-zinc-200">
          <option value="dark">Dark</option>
          <option value="light">Light</option>
        </select>

        <div className="col-span-4 px-10 py-6">
          <h1 className="text-4xl bg-clip-text text-transparent bg-gradient-to-r from-pink-700 to-violet-700 p-1 mb-3">Hello User, Ask Me Anything</h1>
          {
            loader?
              <div role="status">
          ...
          <div className="dark:bg-zinc-800 bg-zinc-300 dark:text-white text-black w-1/2 m-auto border-1 dark:border-zinc-700 border-zinc-400 rounded-4xl flex p-1 pr-5 h-14">
            <input type="text" onChange={(event) => setQuestion(event.target.value)} value={question} onKeyDown={isEnter} placeholder="Ask Me Anything" className="h-full w-full p-3 outline-0" />
